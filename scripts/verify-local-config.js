...
try {
  ...
  if (pkg.bin && pkg.bin['nest-api']) {
    ...
    checks.push({ name: '❌ package.json bin 配置', status: 'FAIL', detail: 'bin 字段未配置' });
  }
  
  if (pkg.scripts['start:local']) {
    checks.push({ name: '✅ start:local 脚本', status: 'OK' });
  } else {
    checks.push({ name: '❌ start:local 脚本', status: 'FAIL', detail: 'start:local 脚本未配置' });
  }
  
  if (pkg.dependencies['better-sqlite3']) {
    checks.push({ name: '✅ better-sqlite3 依赖', status: 'OK' });
  } else {
    checks.push({ name: '⚠️  better-sqlite3 依赖', status: 'WARN', detail: '需要安装: pnpm install better-sqlite3' });
  }
} catch (error) {
  checks.push({ name: '❌ package.json 读取', status: 'FAIL', detail: error.message });
}
...
if (fs.existsSync(startScriptPath)) {
  ...
  checks.push({ name: '❌ start-local.js 脚本', status: 'FAIL', detail: '文件不存在' });
}
...
if (fs.existsSync(envLocalPath)) {
  ...
  checks.push({ name: '⚠️  .env.local 配置文件', status: 'WARN', detail: '可选文件,未找到' });
}
...
if (fs.existsSync(dataDir)) {
  ...
  checks.push({ name: '⚠️  data 目录', status: 'WARN', detail: '将在首次运行时自动创建' });
}
...
if (fs.existsSync(configPath)) {
  ...
  if (configContent.includes('better-sqlite3')) {
    ...
    checks.push({ name: '❌ configuration.ts SQLite 支持', status: 'FAIL', detail: 'SQLite 配置未添加' });
  }
  
  if (configContent.includes('useMemoryCache')) {
    checks.push({ name: '✅ configuration.ts 内存缓存支持', status: 'OK' });
  } else {
    checks.push({ name: '❌ configuration.ts 内存缓存支持', status: 'FAIL', detail: '内存缓存配置未添加' });
  }
} else {
  checks.push({ name: '❌ configuration.ts', status: 'FAIL', detail: '文件不存在' });
}
...
if (fs.existsSync(dbModulePath)) {
  ...
  if (dbModuleContent.includes('better-sqlite3')) {
    ...
    checks.push({ name: '❌ db.module.ts SQLite 支持', status: 'FAIL', detail: 'SQLite 支持未添加' });
  }
} else {
  checks.push({ name: '❌ db.module.ts', status: 'FAIL', detail: '文件不存在' });
}

const cacheConfigPath = path.join(__dirname, '..', 'src', 'cache', 'cache-config.service.ts');
if (fs.existsSync(cacheConfigPath)) {
  ...
  if (cacheConfigContent.includes('useMemoryCache')) {
    ...
    checks.push({ name: '❌ cache-config.service.ts 内存缓存', status: 'FAIL', detail: '内存缓存支持未添加' });
  }
} else {
  checks.push({ name: '❌ cache-config.service.ts', status: 'FAIL', detail: '文件不存在' });
}
